...
- 체크: `(6.5, 8.3) → (10, 11.8) → (17.5, 3.3)`
- 덤벨 바: `y = 18.5`, `x = 7 → 17`
- 안쪽 플레이트: `x = 7`과 `x = 17`, 각각 `y = 16 → 21`
- 바깥 플레이트: `x = 3.8`과 `x = 20.2`, 각각 `y = 17.25 → 19.75`

획 굵기를 감안한 실제 잉크 경계는 체크가 `y = 12.9`에서 끝나고 덤벨이
`y = 14.9`에서 시작한다. **빈 띠 2.0 단위** — 16px로 축소해도 1.3px의 흰 틈이
...
**추인된 조정(amendment).** 실제 첫 렌더에서는 심볼이 타일 안에서 아래로 치우쳐
보였다 — 위 여백이 3.2 단위인데 아래 여백은 0.9 단위였다. 이를 광학적으로
가운데로 옮기기 위해 위 좌표 전체를 y로 **1.0 단위** 위로 이동했다(위에서
말한 ±0.5 한도를 넘는 조정이다). 이동 후 여백은 위 2.2 / 아래 1.9로, 한 덩어리로
보이는 정도의 차이 안에서 시각적으로 균형이 맞는다. `src/components/brandArt.ts`의
좌표는 이 이동을 반영한 값이며, 위 절의 수치도 그 값으로 갱신했다.
