...
9. 지난 기록이 있는 종목을 운동에 추가해 각 세트에 같은 순서의 이전 완료 세션 중량·횟수·세트 유형이 표시되는지 확인하세요. 이 조회(`getPreviousCompletedSessionForExercise`)는 실제 DB에서 별도 확인이 필요하며, 실패하면 "완료 기록 없음"과 "이전 세션 대응 기록 없음"으로 보입니다 — 기록이 있는데도 빈 상태라면 이 조회를 점검합니다.
