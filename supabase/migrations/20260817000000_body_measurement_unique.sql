...
--
-- Supabase SQL 에디터는 여러 statement를 한 번에 제출하면 어차피 하나의
-- 트랜잭션으로 실행하지만, 운영자가 statement를 하나씩 붙여넣어 실행할
-- 경우 파괴적인 DELETE만 커밋되고 제약 추가가 실패한 채로 남을 수 있다.
-- 명시적 트랜잭션으로 그 경우에도 전체가 원자적으로 적용되거나 롤백되게 한다.
