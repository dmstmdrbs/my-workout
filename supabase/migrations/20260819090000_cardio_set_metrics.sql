-- 유산소 세트의 수행 시간과 거리.
--
-- 장비가 'cardio'인 종목은 중량 × 횟수로 기록할 수 없다. 러닝·사이클은
-- 시간과 거리가 기록의 알맹이인데 남길 자리가 없었다.
--
-- 볼륨 합산에는 넣지 않는다. 시간과 거리는 kg에 더할 수 있는 값이 아니라,
-- 섞으면 총 볼륨이 아무 의미 없는 숫자가 된다.
--
-- 컬럼 추가만으로는 부족하다. save_workout_session은 컬럼을 **명시해**
-- INSERT하므로, 함수를 함께 갱신하지 않으면 저장은 성공하는데 시간·거리만
-- 조용히 사라진다. 아래 함수 본문은 20260817120000의 것과 같고, 세트 INSERT의
-- 컬럼 목록과 값 두 줄만 늘렸다.
--
...
alter table public.workout_set_records
  ...
  add column if not exists distance_km numeric(7, 3) check (distance_km is null or distance_km >= 0);
