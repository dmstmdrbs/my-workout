...
const preferencesMock = vi.hoisted(() => ({
  get: vi.fn<(args: { key: string }) => Promise<{ value: string | null }>>(async () => ({ value: null })),
...
describe('플랫폼 영속 저장소', () => {
  ...
  test('localStorage가 더 최신이면 예전 Preferences가 초안을 되돌리지 않는다', async () => {
    localStorage.setItem('draft', 'latest')
    localStorage.setItem(getPersistentMetadataKey('draft'), JSON.stringify({ revision: 20, deleted: false }))
    preferencesMock.get.mockImplementation(async ({ key }: { key: string }) => ({
      value: key === 'draft'
        ? 'stale'
        : JSON.stringify({ revision: 10, deleted: false }),
    }))

    await hydratePersistentStorage(['draft'])

    expect(localStorage.getItem('draft')).toBe('latest')
    expect(preferencesMock.set).toHaveBeenCalledWith({ key: 'draft', value: 'latest' })
  })

  test('최신 삭제 tombstone이 예전 Preferences 초안을 복원하지 못하게 한다', async () => {
    localStorage.setItem(getPersistentMetadataKey('draft'), JSON.stringify({ revision: 20, deleted: true }))
    preferencesMock.get.mockImplementation(async ({ key }: { key: string }) => ({
      value: key === 'draft'
        ? 'stale'
        : JSON.stringify({ revision: 10, deleted: false }),
