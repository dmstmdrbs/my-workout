...
export function ExerciseReorderDialog({ exercises, draggingExerciseId, onClose, onMove, onPointerDown, onPointerUp, onPointerCancel }: ExerciseReorderDialogProps) {
  return <Overlay
    ...
    onClose={onClose}
    presentation="dialog"
    labelledBy="exercise-reorder-title"
    className="exercise-reorder-dialog"
  >
    <header><div><p className="eyebrow">EXERCISE ORDER</p><h2 id="exercise-reorder-title">운동 순서 변경</h2><p>핸들을 길게 눌러 끌어 놓거나, 화살표로 순서를 바꿀 수 있어요.</p></div><button className="icon-button" data-overlay-initial-focus type="button" onClick={onClose} aria-label="순서 변경 닫기"><X size={18} /></button></header>
